...
def check_gemini_api_key():
    """Checks if the Gemini API key is configured and valid before starting the app."""
    logger.info("Checking Gemini API key...")
    api_key = os.getenv('GEMINI_API_KEY')

    if not api_key:
        error_msg = "FATAL: Gemini API key is not set. Please set the GEMINI_API_KEY environment variable."
        logger.error(error_msg)
        raise ValueError(error_msg)

    try:
        genai.configure(api_key=api_key)
        # Perform a lightweight API call to verify the key is working.
        genai.list_models()
        logger.info("Successfully configured and verified the Gemini API key.")
    except Exception as e:
        logger.error(f"FATAL: Gemini API key validation failed. The application will not start. Error: {str(e)}")
        raise e

# Run the check before initializing the FastAPI app
