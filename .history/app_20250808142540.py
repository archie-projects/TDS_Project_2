...
class Config:
    ...
    AIPIPE_MODEL = os.getenv('AIPIPE_MODEL', 'openai/gpt-4o') # Changed to mini version
...
Based on the questions, I need to return a JSON object with these keys:
- "total_revenue": the sum of all revenue/sales
- "top_selling_product": the product that generated the highest revenue
- "sales_by_region_chart": the base64 encoded chart showing sales by region

Generate the final answer as a JSON object with exactly those keys:"""
...
def get_llm_provider():
    if Config.DEFAULT_LLM_PROVIDER == 'gemini' and Config.GEMINI_API_KEY:
        ...
        logger.warning("Defaulting to Ollama provider as configured provider is unavailable.")
        return OllamaProvider(Config.OLLAMA_BASE_URL, Config.OLLAMA_MODEL)
...
async def health_check():
    ...
    try:
        ...
        llm_provider_name = Config.DEFAULT_LLM_PROVIDER
        get_llm_provider() # This will raise an error if config is bad
        return {
            "status": "healthy",
...
if __name__ == "__main__":
    ...
    print(f"🤖 AI Model: {Config.AIPIPE_MODEL if Config.DEFAULT_LLM_PROVIDER == 'aipipe' else Config.OLLAMA_MODEL if Config.DEFAULT_LLM_PROVIDER == 'ollama' else 'gemini-1.5-flash'}")
