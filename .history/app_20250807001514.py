...
class DataAnalystAgent:
    ...
    async def _create_plan(self, questions: str, context: Dict) -> Dict:
        # This prompt is now simplified. The AI only needs to know that a table named 'data_table' will exist.
        prompt = f"""
        ...
        **Your Task:**
        ...
        When creating a plan, use generic column names like 'Rank', 'Year', 'Gross', 'Peak', 'Title', 'ProductName', 'UnitsSold', 'Price'. The execution engine will map them correctly.

        **Supported Actions & Params:**
        - "scrape_website": {{"url": "URL to scrape"}}
        - "run_duckdb_sql": {{"query": "SQL query to run on the 'data_table'."}}
        - "generate_plot": {{"plot_type": "bar_chart or scatterplot", "x_col": "x-axis column", "y_col": "y-axis column", "options": {{"color": "red", "style": "dotted"}}}}
        """
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Keep original object columns for plotting labels, etc.
                ...
                try:
                    if action == "run_duckdb_sql":
                        ...
                        if params.get("plot_type") == "bar_chart":
                            # For bar charts, we plot counts or values against categories
...
def create_test_files():
    """Creates the test files needed for the test."""
