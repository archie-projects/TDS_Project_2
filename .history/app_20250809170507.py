...
class UniversalDataAnalyst:
      ...
      try:
          ...
          if isinstance(result, dict):
              ...
              for key, value in list(result.items()):
                  # If the value is a dict with a type → it's a visualization request
                  if isinstance(value, dict) and "type" in value:
                      img_b64 = self.create_visualization(value, context)
                      if img_b64:
                          result[key] = img_b64
                  # If the value is exactly "visualization_request"
                  elif isinstance(value, str) and value.lower() == "visualization_request":
                      # If Gemini included a central "visualization_requests" dict
                      if "visualization_requests" in result and key in result["visualization_requests"]:
                          viz_req = result["visualization_requests"][key]
                          img_b64 = self.create_visualization(viz_req, context)
                          if img_b64:
                              result[key] = img_b64

              # Remove the extra "visualization_requests" key if it exists
              if "visualization_requests" in result:
                  del result["visualization_requests"]
    ...
    def create_visualization(self, viz_request: Dict, context: Dict) -> Optional[str]:
      ...
      try:
          ...
          # --- Save to PNG and shrink until under 100kB ---
          ...
          while len(img_data) > 100 * 1024 and dpi > 30:
              dpi = int(dpi * 0.8)  # lower resolution
