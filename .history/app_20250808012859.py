                        ...
                        if 's3://' in data_source or 's3://' in str(query):
                            ...
                            if query and 'year=2019-2022' in query:
                                ...
                                for year in [2019, 2020, 2021, 2022]:
                                    union_queries.append(f"SELECT * FROM read_parquet('{base_query.split('FROM read_parquet(')[1].strip(\"'\")}')".replace('year={}', f'year={year}'))
                            ...
                            try:
                                ...
                                alt_queries = [
                                    "SELECT * FROM read_parquet('s3://indian-high-court-judgments/metadata/parquet/year=2019/court=*/bench=*/metadata.parquet?s3_region=ap-south-1') UNION ALL SELECT * FROM read_parquet('s3://indian-high-court-judgments/metadata/parquet/year=2020/court=*/bench=*/metadata.parquet?s3_region=ap-south-1') UNION ALL SELECT * FROM read_parquet('s3://indian-high-court-judgments/metadata/parquet/year=2021/court=*/bench=*/metadata.parquet?s3_region=ap-south-1') UNION ALL SELECT * FROM read_parquet('s3://indian-high-court-judgments/metadata/parquet/year=2022/court=*/bench=*/metadata.parquet?s3_region=ap-south-1')",
                                    "SELECT * FROM read_parquet('s3://indian-high-court-judgments/metadata/parquet/**/*.parquet?s3_region=ap-south-1') WHERE year BETWEEN 2019 AND 2022"
                                ]
                                for alt_query in alt_queries:
                                    try:
                                        result_df = con.execute(alt_query).df()
                                        main_df = self._clean_dataframe(result_df)
                                        con.register('main_data', main_df)
                                        results[step_id] = {'type': 'dataframe', 'shape': main_df.shape}
                                        logger.info(f"Alternative query succeeded with shape: {main_df.shape}")
                                        break
                                    except Exception as e2:
                                        logger.error(f"Alternative query failed: {e2}")
                                        continue
                        else:
                            if query:
