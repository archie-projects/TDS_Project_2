...
class UniversalDataAnalyst:
    ...
    try:
        ...
        def guess_fallback_viz():
            """Guess a visualization request if Gemini didn't provide details."""
            if context.get('file_data'):
                for file_info in context['file_data'].values():
                    if file_info['type'] == 'dataframe':
                        df_guess = pd.DataFrame(file_info['data'])
                        # Guess network graph if exactly two columns
                        if df_guess.shape[1] == 2:
                            return {"type": "network_graph"}
                        # Else pick first two numeric cols for scatter
                        num_cols = df_guess.select_dtypes(include='number').columns
                        if len(num_cols) >= 2:
                            return {"type": "scatter", "x": num_cols[0], "y": num_cols[1]}
                        # Else default to bar chart
                        if df_guess.shape[1] >= 2:
                            return {"type": "bar", "x": df_guess.columns[0], "y": df_guess.columns[1]}
            return None
    ...
    def create_visualization(self, viz_request: Dict, context: Dict) -> Optional[str]:
      ...
      try:
          ...
          if viz_type == "scatter":
              ...
              x_col, y_col = viz_request.get("x"), viz_request.get("y")
              if x_col in df.columns and y_col in df.columns:
                  color = viz_request.get("bar_color", "blue")
              ...
              x_col, y_col = viz_request.get("x"), viz_request.get("y")
              if x_col in df.columns and y_col in df.columns:
                  ax.plot(df[x_col], df[y_col], marker='o', color=viz_request.get("line_color", "blue"))
              else:
