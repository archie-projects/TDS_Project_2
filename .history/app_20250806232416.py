...
class DataAnalystAgent:
    ...
    async def _create_plan(self, questions: str, context: Dict) -> Dict:
        ...
        **Your Task:**
        ...
        If data is scraped, it will be registered as a DuckDB table named 'data_table'. All subsequent SQL steps MUST query this 'data_table'.
    ...
    async def _execute_plan(self, plan: Dict) -> Dict:
        ...
        with duckdb.connect(database=':memory:', read_only=False) as con:
            ...
            # This will hold the primary dataframe for the session
            main_df = None
                ...
                try:
                    if action == "scrape_website":
                        ...
                        if table:
                            ...
                            execution_results[step_key] = f"Scraped data. Columns: {main_df.columns.tolist()}"
                    
                    elif action == "run_duckdb_sql":
                        if main_df is None:
                            raise ValueError("No data available to query. A 'scrape_website' or file upload step must run first.")
                        
                        query = params["query"]
                        
                        # Find all potential column names in the query
                        ...
                        if main_df is None:
                            raise ValueError("No data available to plot. A 'scrape_website' or file upload step must run first.")
                        
                        df = main_df
                        fig, ax = plt.subplots(figsize=(8, 5))
                        
                        x_col_actual_name = self._get_actual_column_name(df.columns, params["x_col"]).strip('"')
                        y_col_actual_name = self._get_actual_column_name(df.columns, params["y_col"]).strip('"')
                        
                        valid_data = df[[x_col_actual_name, y_col_actual_name]].dropna()

                        ax.scatter(valid_data[x_col_actual_name], valid_data[y_col_actual_name], alpha=0.6)
                        
                        if "options" in params and not valid_data.empty:
                            z = np.polyfit(valid_data[x_col_actual_name], valid_data[y_col_actual_name], 1)
                            p = np.poly1d(z)
                            ax.plot(valid_data[x_col_actual_name], p(valid_data[x_col_actual_name]), color=params["options"].get("color", "red"), linestyle=params["options"].get("style", "dotted"))
                        
                        ax.set_xlabel(x_col_actual_name); ax.set_ylabel(y_col_actual_name); plt.grid(True, alpha=0.3)
                        plt.tight_layout()
                        buf = io.BytesIO()
