...
class DataAnalystAgent:
    ...
    async def _execute_plan(self, plan: Dict, context: Dict) -> Dict:
        ...
        con.execute("INSTALL parquet; LOAD parquet;") # Ensure parquet is loaded
        ...
        try:
            ...
            for file_info in context['files']:
                if file_info.get('type') == 'tabular' and file_info.get('data') is not None:
                    try:
                        ...
                        table_name = f"file_{re.sub(r'[^\\w]', '_', file_info['filename'])}"
            ...
            for step in plan.get('steps', []):
                ...
                try:
                    # The 'load_data' action loads data, often from a remote source
                    if action == 'load_data':
                        query = params.get('query')
                        if query:
                            logger.info(f"Loading data with query: {query}")
                            result_df = con.execute(query).df()
                            # This loaded data becomes the main dataframe for subsequent steps
                            main_df = self._clean_dataframe(result_df)
                            con.register('main_data', main_df) # Register as 'main_data' for SQL steps
                            results[step_id] = {
                                'type': 'dataframe',
                    ...
                    # The 'run_sql' action runs a query, assuming data is already loaded
                    elif action == 'run_sql':
                        query = params.get('query', '')
                        if query:
                            # In case a query references 'main_data' but it's not loaded yet
                            if 'main_data' not in con.list_tables() and main_df is None:
                                logger.error("No data loaded to run SQL query on.")
                                raise ValueError("SQL query requires data to be loaded first.")
                            
                            logger.info(f"Executing SQL: {query}")
                            # The table name 'main_data' is used by convention in the plan
                            result_df = con.execute(query.replace("FROM metadata", "FROM main_data")).df()
                            results[step_id] = {
                                'type': 'dataframe',
                        ...
                        con.register('main_data', main_df) # Re-register the cleaned dataframe
                        ...
                        # Check if data should come from a previous step
                        analysis_df = main_df
