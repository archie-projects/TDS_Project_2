...
class DataAnalystAgent:
    ...
    async def _create_plan(self, questions: str, context: Dict) -> Dict:
        ...
        **Your Task:**
        ...
        For tasks requiring aggregation (e.g., "total revenue by region"), you MUST use a GROUP BY clause in your SQL query.
        For tasks like finding the "best" or "most," use an SQL query with ORDER BY and LIMIT 1.
        When creating a plan, use generic column names like 'Rank', 'Year', 'Gross', 'Peak', 'Title', 'ProductName', 'UnitsSold', 'Price', 'Region', 'Discount'. The execution engine will map them correctly.

        **Supported Actions & Params:**
        - "scrape_website": {{"url": "URL to scrape"}}
        - "run_duckdb_sql": {{"query": "SQL query to run on the 'data_table'."}}
        - "generate_plot": {{"plot_type": "bar_chart or scatterplot", "x_col": "x-axis column", "y_col": "y-axis column", "options": {{}}, "data_source_step": "Optional: The step_key of the SQL query that generates the data for this plot (e.g., 'step_3_run_duckdb_sql')."}}
        """
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        object_columns = [col for col in df.columns if any(name.lower() in col.lower() for name in ['product', 'region', 'title'])]
        ...
        if discount_col:
            df[discount_col] = df[discount_col].fillna(0)
            ...
            for i, step in enumerate(plan.get("plan", [])):
                ...
                try:
                    if action == "run_duckdb_sql":
                        ...
                        # A more robust way to find and replace column names
                        def replace_col(match):
                            col_name = match.group(0)
                            try:
                                return self._get_actual_column_name(main_df.columns, col_name)
                            except ValueError:
                                return col_name # Keep original if no match
                        
                        # Find all words that could be column names (more precise regex)
                        potential_cols = re.findall(r'\b[A-Z][a-zA-Z_]*\b', query)
                        for col in set(potential_cols):
                           query = re.sub(r'\b' + col + r'\b', lambda m: replace_col(m), query)

                        logging.info(f"Executing rewritten SQL: {query}")
                        result_df = con.execute(query).df()
                        execution_results[step_key] = result_df.to_dict(orient='records')
                        con.register(f'result_{i+1}', result_df)

                    elif action == "generate_plot":
                        plot_data_source = main_df
                        # --- FIX: Use the result of a previous SQL step if specified ---
                        if "data_source_step" in params and params["data_source_step"] in execution_results:
                            source_result = execution_results[params["data_source_step"]]
                            if isinstance(source_result, list):
                                plot_data_source = pd.DataFrame(source_result)
                                logging.info(f"Using data from {params['data_source_step']} for plotting.")
                        
                        if plot_data_source is None: raise ValueError("No data available to plot.")
                        ...
                        if params.get("plot_type") == "bar_chart":
                            ax.bar(df[x_col_actual], df[y_col_actual])
                            plt.xticks(rotation=45, ha="right")
                        else: # Default to scatterplot
    ...
    async def _synthesize_answer(self, questions: str, results: Dict) -> Any:
        ...
        Use the 'Execution Results' to find the values. If a step has an error, state that in a user-friendly way. For example, if a value is missing, return null.
