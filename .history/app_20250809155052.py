...
class UniversalDataAnalyst:
    ...
    def generate_analysis_response(self, questions: str, context: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            """Creates a plot based on a request from the LLM and returns a base64 string under 100kB."""
            ...
            try:
                ...
                # Save as PNG and compress until under 100kB
                buf = BytesIO()
                plt.savefig(buf, format='png', dpi=100)  # start with reasonable DPI
                plt.close(fig)

                image_data = buf.getvalue()
                while len(image_data) > 100 * 1024:  # 100 kB limit
                    buf = BytesIO()
                    ...
                    ax.set_title(viz_request.get("title", f"{y_col} vs. {x_col}"))
                    plt.tight_layout()
                    plt.savefig(buf, format='png', dpi=80)  # reduce DPI further
                    plt.close(fig)
                    image_data = buf.getvalue()
