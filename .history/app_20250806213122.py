...
class DataAnalystAgent:
    ...
    async def _create_plan(self, questions: str, context: Dict) -> Dict:
        ...
        **Your Task:**
        ...
        If data is scraped, it will be registered as a DuckDB table named 'data_table'. Subsequent SQL steps MUST query 'data_table'.
                ...
                try:
                    if action == "scrape_website":
                        ...
                        if table:
                            ...
                            # Register the dataframe as a table that can be queried by SQL
                        ...
                        for generic_name in ['Gross', 'Year', 'Rank', 'Peak', 'Title']:
                            try:
                                ...
                                # Use regex to replace whole word only, case-insensitively
                                query = re.sub(r'\b' + re.escape(generic_name) + r'\b', f'"{actual_name}"', query, flags=re.IGNORECASE)
                            except ValueError:
                                pass 
                        ...
                        x_data = pd.to_numeric(df[x_col_actual], errors='coerce')
                        y_data = pd.to_numeric(df[y_col_actual], errors='coerce')
                        
                        # Drop rows where either x or y is NaN after conversion
                        valid_data = pd.DataFrame({'x': x_data, 'y': y_data}).dropna()

                        ax.scatter(valid_data['x'], valid_data['y'], alpha=0.6)
                        
                        if "options" in params and not valid_data.empty:
                            z = np.polyfit(valid_data['x'], valid_data['y'], 1)
                            p = np.poly1d(z)
                            ax.plot(valid_data['x'], p(valid_data['x']), color=params["options"].get("color", "red"), linestyle=params["options"].get("style", "dotted"))
