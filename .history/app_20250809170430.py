...
class UniversalDataAnalyst:
    ...
    def process_visuals(obj):
    """Recursively replace visualization requests with base64 strings."""
    if isinstance(obj, dict):
        # Case 1: dict already has type → direct viz request
        if "type" in obj:
            return self.create_visualization(obj, context)

        # Case 2: dict contains visualization_request key
        if "visualization_request" in obj:
            viz_req = obj["visualization_request"]
            # If Gemini gave full params
            if isinstance(viz_req, dict) and "type" in viz_req:
                return self.create_visualization(viz_req, context)
            # If Gemini gave no params, fallback
            elif isinstance(viz_req, str) and viz_req.lower() == "visualization_request":
                guessed = guess_fallback_viz()
                if guessed:
                    logger.info(f"Fallback guess for nested visualization_request: {guessed}")
                    return self.create_visualization(guessed, context)

        # Recurse for other keys
        return {k: process_visuals(v) for k, v in obj.items()}

    elif isinstance(obj, list):
        return [process_visuals(i) for i in obj]

    elif isinstance(obj, str) and obj.lower() == "visualization_request":
        guessed = guess_fallback_viz()
        if guessed:
            logger.info(f"Fallback guess for plain visualization_request: {guessed}")
            return self.create_visualization(guessed, context)
        return obj
    ...
    def generate_analysis_response(self, questions: str, context: Dict[str, Any]) -> Any:
      ...
      try:
          ...
          def process_visuals(obj):
              """Recursively replace visualization requests with base64 strings."""
              if isinstance(obj, dict):
                  # Case 1: dict directly describing a visualization
                  if "type" in obj:
                      return self.create_visualization(obj, context)
                  # Case 2: dict containing a single key 'visualization_request'
                  if "visualization_request" in obj and isinstance(obj["visualization_request"], dict):
                      return self.create_visualization(obj["visualization_request"], context)
                  # Otherwise, recurse into each value
                  return {k: process_visuals(v) for k, v in obj.items()}
              elif isinstance(obj, list):
                  return [process_visuals(i) for i in obj]
              elif isinstance(obj, str) and obj.lower() == "visualization_request":
                  # String placeholder — can't process without details, so leave as is
                  return obj
              else:
                  return obj
    ...
    def create_visualization(self, viz_request: Dict, context: Dict) -> Optional[str]:
      """Generically create a plot from viz_request and return base64 PNG under 100kB."""
      ...
      try:
          ...
          if viz_type == "scatter":
              x, y = viz_request.get("x"), viz_request.get("y")
              ...
              x, y = viz_request.get("x"), viz_request.get("y")
              if x in df.columns and y in df.columns:
                  ax.bar(df[x], df[y], color=viz_request.get("bar_color", "blue"))
              else:
                  return None
              ...
              x, y = viz_request.get("x"), viz_request.get("y")
              if x in df.columns and y in df.columns:
                  ax.plot(df[x], df[y], marker='o', color=viz_request.get("color", "blue"))
              else:
                  return None

          elif viz_type == "pie":
              labels, values = viz_request.get("labels"), viz_request.get("values")
              ...
              x = viz_request.get("x")
              if x in df.columns:
                  ax.hist(df[x].dropna(), bins=viz_request.get("bins", 10), color=viz_request.get("color", "blue"))
                  ax.set_xlabel(x)
          ...
          ax.set_title(viz_request.get("title", "Visualization"))
          plt.tight_layout()

          # Base64 encode under 100kB
          def encode_png(fig_obj, dpi_val):
              buf = BytesIO()
              fig_obj.savefig(buf, format='png', dpi=dpi_val)
              return buf.getvalue()

          img_data = encode_png(fig, 100)
          dpi = 100
