...
class DataAnalystAgent:
    ...
    async def _create_plan(self, questions: str, context: Dict) -> Dict:
        ...
        **Supported Actions & Params:**
        ...
        - "generate_plot": {{"plot_type": "bar_chart or scatterplot", "x_col": "x-axis column", "y_col": "y-axis column", "options": {{}}}}
        """
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Identify columns that should remain as objects (text)
        ...
        for col in df.columns:
            if col not in object_columns:
                # This handles columns that are already numeric as well
                cleaned_series = df[col].astype(str).str.replace(r'[$,]', '', regex=True)
                df[col] = pd.to_numeric(cleaned_series, errors='coerce')
        
        # Handle specific cases like missing discounts
            ...
            for i, step in enumerate(plan.get("plan", [])):
                ...
                try:
                    if action == "run_duckdb_sql":
                        ...
                        # Store the result of the query to be used by other steps (like plotting)
                        execution_results[step_key] = result_df.to_dict(orient='records')
                        # Also register the result as a new table for potential follow-up queries
                        ...
                        # --- FIX: Use the result of a previous SQL step if available ---
                        # This allows plotting of aggregated data.
                        plot_data_source = main_df
                        if "data_source_step" in params:
                            source_result = execution_results.get(params["data_source_step"])
                            if source_result and isinstance(source_result, list):
