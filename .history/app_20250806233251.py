...
class DataAnalystAgent:
    ...
    async def _execute_plan(self, plan: Dict, uploaded_files: Dict) -> Dict:
        ...
        with duckdb.connect(database=':memory:', read_only=False) as con:
            ...
            # This will hold the primary dataframe for the session, loaded from scraping or a file
            main_df = None
            ...
            for i, step in enumerate(plan.get("plan", [])):
                ...
                try:
                    # Load data from the first available source (scrape or file)
                    if main_df is None:
                        if "scrape_website" in [s['action'] for s in plan.get("plan", [])]:
                            # This block will only run if scraping is part of the plan
