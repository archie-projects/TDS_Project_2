...
class UniversalDataAnalyst:
    ...
    def analyze_request(self, questions: str, files: List[Dict[str, Any]]) -> Any:
        ...
        try:
            ...
            if "indian high court judgement dataset" in questions.lower() and "s3://" in questions.lower():
                logger.info("Indian High Court dataset query detected. Generating and executing DuckDB query.")
                # Step 1: Generate a DuckDB query from the user's questions
                ...
                if duckdb_query:
                    ...
                    if query_result_df is not None:
                        # Add results to the context for the next step
            ...
            # Step 3: Generate the final JSON answer using all available context
    ...
    def generate_duckdb_query(self, user_prompt: str) -> Optional[str]:
        ...
        3.  If the user asks about the time between two dates (e.g., 'date_of_registration' and 'decision_date'), calculate the difference in days directly in the SQL query. Name the calculated column `days_of_delay`. Use `(CAST(decision_date AS DATE) - CAST(date_of_registration AS DATE))` for this calculation.
        4.  The query MUST start with `INSTALL httpfs; LOAD httpfs; INSTALL parquet; LOAD parquet;`
        5.  Filter the data as needed (e.g., by year or court code).
        6.  Return ONLY the raw SQL query, without any explanation, markdown, or comments.

        Example for calculating delay:
        SELECT year, (CAST(decision_date AS DATE) - CAST(date_of_registration AS DATE)) AS days_of_delay FROM read_parquet(...) WHERE court_code = '33_10'
        """
        try:
    ...
    def build_analysis_prompt(self, questions: str, context: Dict[str, Any]) -> str:
        ...
        if not any(key in context for key in ['file_data', 'scraped_data', 'duckdb_query_result']):
